// Write a program that first gets a list of integers from input. The input begins with an integer indicating 
// the number of integers that follow. Then, get the last value from the input, which indicates a threshold. Output 
// all integers less than or equal to that last threshold value.

// Ex: If the input is:
// 5 50 60 140 200 75 100
// the output is:
// 50,60,75,

// The 5 indicates that there are five integers in the list, namely 50, 60, 140, 200, and 75. The 100 indicates that 
// the program should output all integers less than or equal to 100, so the program outputs 50, 60, and 75.
// For coding simplicity, follow every output value by a comma, including the last one.
// Such functionality is common on sites like Amazon, where a user can filter results.

// + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + + 

// Title: less_than_last_value.cpp
// Course: Computational Problem Solving I (CPET-121)
// Developer: Muhfasul Alam
// Date: July 21, 2021
// Description: prints the values less than the last value in a vector

#include <iostream>
#include <vector>
using namespace std;

int main() {
   int userNum; // input values
   int maxNum;
   int i;
   
   // stores the length from first input
   int length;
   cin >> length;
   
   vector<int> userInts(length); // stores inputs in a vector
   
   for (i = 0; i < length; ++i){
      cin >> userNum;
      userInts.at(i) = userNum;
   }
   
   cin >> maxNum; // gets the max value
   
   // prints values lower than the max values
   for (i = 0; i < length; ++i) {
      if (userInts.at(i) < maxNum) {
         cout << userInts.at(i) << ",";
      }
   }
   
   cout << endl;

   return 0;
}
